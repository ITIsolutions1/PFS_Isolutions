...
<div class="container py-4">
    ...
    {{-- Tombol kembali --}}
    <div class="mt-3">
        <a href="{{ $previousUrl ?? route('proposals.index') }}" class="btn btn-outline-danger">
    ...
    <h3 class="fw-bold text-danger mb-4 mt-5">
        <i class="bi bi-people-fill me-2"></i> Detail Lead: {{ $proposal->lead->crm->name ?? '-' }}
    </h3>
    ...
    <div class="card shadow-sm rounded-4">
        <div class="card-body">
            ...
            <ul class="nav nav-tabs mb-3" id="leadTab" role="tablist">
                <li class="nav-item">
                    <button class="nav-link active" id="proposal-tab" data-bs-toggle="tab" data-bs-target="#proposal" type="button">Informasi Proposal</button>
                </li>
                <li class="nav-item">
                    <button class="nav-link " id="contact-tab" data-bs-toggle="tab" data-bs-target="#contact" type="button">Informasi Contact</button>
                </li>
                <li class="nav-item">
...
<div class="tab-pane fade" id="persona" role="tabpanel">
    ...
    @if($proposal->lead->persona)
        ...
        @include('leads.persona.persona_list', ['persona' => $proposal->lead->persona])
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setupEditableSection(
        '#contact',
        '/admin/crm/{{ $proposal->lead->crm->id }}/update-multiple',
        '.edit-mode',
        false
    );
...
setupEditableSection(
    '#persona',
    `/personas/{{ $proposal->lead->persona->id }}/update-multiple`,
    '.edit-mode-persona',
    false
);
@endif
   ...
   setupEditableSection(
    '#followup',
    '/followups/{{ $proposal->lead->id }}/bulk-update',
    '.edit-mode-follow',
    true
);
...
<script>
    ...
    fetch(`/leads/{{ $proposal->lead->id }}/followups`, {
        method: "POST",
